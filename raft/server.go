...
func (s *server) setState(state string) {
	...
	prevState := s.state
	prevLeader := s.leader
	log.Print("preState:", prevState)
	log.Print("newState:", state)
...
func (s *server) QuorumSize() int {
	if (s.MemberCount() % 2) > 0 {
		return (s.MemberCount() / 2) + 1
	} else {
		return s.MemberCount() / 2
	}
}
		...
		case e := <-s.c:
			...
			case JoinCommand:
				...
				if s.log.currentIndex() == 0 && req.NodeName() == s.Name() {
					s.debugln("self join and promote to leader")
					s.setState(Leader)
